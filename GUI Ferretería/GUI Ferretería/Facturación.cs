...
namespace GUI_Ferretería
{
    public partial class Facturación : Form
    {
        ...
        // CONSTRUCTOR
        // ▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬
        public Facturación(Usuario usuarioActivo)
        {
            InitializeComponent();
        ...
        private void cargarDatos2()
        {
            try
            {
                ...
                OracleCommand miComando = new OracleCommand("SP_DETALLE_INVENTARIO", miConexión);
        ...
        private void btnbuscar_Click_1(object sender, EventArgs e)
        {
            ...
            OracleCommand cmd = new OracleCommand("select * from VDETALLEINVENTARIO  WHERE DESCRIPCION like'%" + TXTBUSCAR.Text + "%'", conn);
            conn.Open();
        ...
        private void textBox6_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (Convert.ToInt32(e.KeyChar) == 13)
            {
                try
                {
                    ...
                    miComando.CommandType = CommandType.StoredProcedure;
                    miComando.Parameters.Add("P_IDENTIDAD", OracleDbType.Varchar2).Value = textBox6.Text;
                    ...
                    resultado.Read();
                    textBox5.Text = resultado.GetString(0);

                    miConexión.Close();

                    OracleCommand miComando2 = new OracleCommand("SP_BUSCAR_CODIGO_CLIENTE", miConexión);
                    miComando2.CommandType = CommandType.StoredProcedure;
                    miComando2.Parameters.Add("P_IDENTIDAD", OracleDbType.Varchar2).Value = textBox6.Text;
                    miComando2.Parameters.Add("P_RESULTADO", OracleDbType.RefCursor, ParameterDirection.Output);
                    miConexión.Open();

                    OracleDataReader resultado2 = miComando2.ExecuteReader();

                    resultado2.Read();
                    codigocliente = resultado2.GetInt32(0);
        ...
        {
            if (dgvDatos.Rows.Count == 0 || codigocliente == 0)
            {

                MessageBox.Show("No ha seleccionado productos o no ha validado los datos del cliente.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                MessageBox.Show("Para validar los datos del cliente introduzca un número de identidad y presione ENTER,\nasegúrese de que el cliente esté registrado.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            ...
            else
            {
                ...
                OracleCommand miComando = new OracleCommand("SP_GUARDAR_DETALLE_FACTURA", miConexión);
                miComando.CommandType = CommandType.StoredProcedure;
                //miComando.CommandType = CommandType.StoredProcedure; // ?????????

                miComando.Parameters.Add("CODIGOSU", OracleDbType.Int32).Value = usuarioActivo.codigoSucursal;
                //miComando.Parameters.Add("FECHA", OracleDbType.Date).Value = DateTime.Today;
                miComando.Parameters.Add("DESCUENTO", OracleDbType.Varchar2).Value = desc.ToString().Trim();
                miComando.Parameters.Add("SUBTOTAL", OracleDbType.Varchar2).Value = Stotal.ToString().Trim();
                ...
                miComando.Parameters.Add("CODIGOEMP", OracleDbType.Int32).Value = usuarioActivo.codigoEmpleado;
                miComando.Parameters.Add("CODIGOCARGO", OracleDbType.Int32).Value = usuarioActivo.codigoCargo;
                miComando.Parameters.Add("CODIGOCLIENTE", OracleDbType.Int32).Value = codigocliente;

                miComando.ExecuteNonQuery();


                OracleCommand miComando2 = new OracleCommand("SELECT SQ_CODIGO_FACTURA.CURRVAL FROM DUAL", miConexión);
                OracleDataReader resultado2 = miComando2.ExecuteReader();
                resultado2.Read();
                this.codigoFactura = resultado2.GetInt32(0);

                OracleCommand miComando3 = new OracleCommand("SP_GUARDAR_ARTICULOS_FACTURA", miConexión);
                ...
                foreach (DataGridViewRow fila in this.dgvDatos.Rows)
                {
                    ...
                    miComando3.Parameters.Add(" CODIGOFA", OracleDbType.Int32).Value = this.codigoFactura;
                    ...
                    miComando3.Parameters.Add("SUBT", OracleDbType.Int32).Value = Convert.ToUInt32(fila.Cells["COSTO_ACUMULADO"].Value.ToString().Trim());
                ...
                MessageBox.Show("Los datos han sido almacenados\ncon el número de factura: " + this.codigoFactura.ToString(), "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
                miConexión.Close();
